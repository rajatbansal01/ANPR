...
def detect(frame):
    # changing RGB to gray for better classification
...
def csv_updater(v_number):
    ...
    with open('data_new.csv', 'a', newline='') as f:
        csv_file = DictWriter(
            f, fieldnames=["date", "v_number", 'plate_path', 'face_path', "status"])
        csv_file.writerows([
    ...
    f.close()
# status check if uietian or not
...
# plate path updater
def plate_updater(v_number):
    with open('data_new.csv', 'a', newline='') as f:
        csv_file = DictWriter(f, fieldnames=["date", "v_number"])
        csv_file.writerows([
            {'date': [time.asctime(time.localtime(
                time.time()))], 'v_number': v_number}
        ])
    f.close()
